...
public:
	...
	Image LoadImageFromFile(const std::string& path)
	{
		...
		void* data;
		vmaMapMemory(m_Allocator, stagingBuffer._allocation, &data);
		...
		vk::Format image_format = vk::Format::eR8G8B8A8Srgb;//	VK_FORMAT_R8G8B8A8_SRGB;
		...
		dimg_info.mipLevels = 1;// = vk::ImageType::e2D;
		dimg_info.arrayLayers = 1;// = vk::ImageType::e2D;
		dimg_info.samples = vk::SampleCountFlagBits::e1;// = vk::ImageType::e2D;
		...
		vmaCreateImage(m_Allocator, reinterpret_cast<VkImageCreateInfo*>(&dimg_info),
