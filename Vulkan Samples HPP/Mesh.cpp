	...
	const std::string& directory)
{
	...
	if (colors = mesh->HasVertexColors(0))
		m_VertexBuffer.AddAttribute(VertexColor_u16vec4 {});
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		if (colors)
		{
			...
			uint16_t* r = reinterpret_cast<uint16_t*>(&color.r);
			uint16_t* g = reinterpret_cast<uint16_t*>(&color.r) + 1;
			uint16_t* b = reinterpret_cast<uint16_t*>(&color.r) + 2;
			uint16_t* a = reinterpret_cast<uint16_t*>(&color.r) + 3;
			...
			//}
			m_VertexBuffer.AddElement<VertexColor_u16vec4>(*r, *g, *b,*a );
